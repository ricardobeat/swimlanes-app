...
class ServiceAPI {
  ...
  async getBoard(boardId: string): Promise<Task[]> {
  ...
  subscribe(boardId: string, onEvent: (event: ServerEvent) => void): EventSource {
    const evtSource = new EventSource(`${API_URL}/events?stream=${boardId}`);
